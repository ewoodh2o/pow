...
# Set up the environment.
      ...
      POWD_PLIST_PATH="/Library/LaunchAgents/cx.pow.powd.plist"
      POW_CONFIG_PATH="$HOME/.powconfig"

# Fail fast if Pow isn't present.

      if [[ ! -d "$POW_CURRENT_PATH" ]] && [[ ! -a "$POWD_PLIST_PATH" ]]; then
...
# Unload cx.pow.powd from launchctl and remove the plist.
      ...
      sudo launchctl unload "$POWD_PLIST_PATH" 2>/dev/null || true
